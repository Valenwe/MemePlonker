...
def render_canvas_image(canvas: ttk.Canvas) -> Image.Image:
    ...
    for obj in config.objects:
        ...
        if isinstance(obj.data, str):
            ...
            # Draw the text exactly where Tk places it: horizontally centered and
            # sitting on the baseline (Tk's ascent below the line-box top). This
            # matches the preview's vertical position. A small pad avoids clipping.
            pad = 2
            ...
            text_draw.text((box_w / 2, baseline), text, font=pil_font, fill=fill, anchor="ms")
...
def save_canvas_as_image(canvas: ttk.Canvas):
    ...
    if has_gif and file_path.lower().endswith(".gif"):
        ...
        _save_animated_gif(frames, duration, file_path)
    else:
        render_canvas_image(canvas).save(file_path, format="PNG", subsampling=0, quality=100)
...
def copy_canvas_to_clipboard(canvas: ttk.Canvas):
    ...
    if sys.platform.startswith("win"):
        copied = _copy_windows(canvas, has_gif)
    elif sys.platform.startswith("linux"):
        copied = _copy_linux(canvas, has_gif)
    else:
...
def _gif_bytes(canvas: ttk.Canvas) -> bytes:
    """Encode the canvas GIF timeline to animated-GIF bytes."""
    frames, duration = render_canvas_gif_frames(canvas)
...
def _copy_linux(canvas: ttk.Canvas, has_gif: bool) -> bool:
    if has_gif:
        return _bytes_to_clipboard_linux(_gif_bytes(canvas), "image/gif")
    with io.BytesIO() as output:
        render_canvas_image(canvas).save(output, "PNG")
        return _bytes_to_clipboard_linux(output.getvalue(), "image/png")


def _copy_windows(canvas: ttk.Canvas, has_gif: bool) -> bool:
    import ctypes

    # A DIB of the current frame is the universal fallback every app understands.
    with io.BytesIO() as output:
        render_canvas_image(canvas).convert("RGB").save(output, "BMP")
        dib = output.getvalue()[14:]  # strip the 14-byte BMP file header
    CF_DIB = 8
    entries: list[tuple[int, bytes]] = [(CF_DIB, dib)]

    if has_gif:
        gif_data = _gif_bytes(canvas)
...
def _bytes_to_clipboard_linux(payload: bytes, mime: str) -> bool:
    ...
    if shutil.which("wl-copy"):
        command = ["wl-copy", "-t", mime]
    elif shutil.which("xclip"):
        command = ["xclip", "-selection", "clipboard", "-t", mime]
    else:
        logger.error("Install 'xclip' (X11) or 'wl-copy' (Wayland) to copy images.")
        return False
    ...
    try:
        ...
        logger.error("Clipboard tool '%s' failed.", command[0])
        return False
